@props(['storeId', 'avatar', 'firstname', 'lastname', 'rating', 'availability', 'skills'])
...
<div class="w-full max-w-sm mx-auto relative overflow-hidden shadow-lg group">
    <div class="flex flex-col">
        ...
        <!-- Name and Skills (visible by default) -->
        <div class="text-white p-4">
            <h3 class="text-xl font-bold">{{ $firstname }} {{ $lastname }}</h3>
            <x-star-rating :rating="$rating" />
            <div class="mt-2">
                @foreach (array_slice($skills_array, 0, 3) as $index => $skill)
                    <span class="text-xs sm:text-sm">{{ $skill }}@if($index < count(array_slice($skills_array, 0, 3)) - 1),@endif</span>
                @endforeach
    ...
    <div class="absolute inset-0 bg-black bg-opacity-95 text-white p-4 sm:p-6 flex flex-col justify-between transform translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-in-out">
        <div>
            <h3 class="text-xl font-bold mb-2">{{ $firstname }} {{ $lastname }}</h3>
            <x-star-rating :rating="$rating" />
        ...
        <div class="flex items-center justify-between">
            ...
            <a href="/marketplace/store/{{ $storeId }}" class="text-sm sm:text-base text-yellow-400 hover:text-yellow-600 transition-colors duration-200">
                check me out &#10230;
            </a>
        </div>
    </div>
